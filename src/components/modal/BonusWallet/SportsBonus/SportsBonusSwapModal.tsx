...
import { SwapSend } from "@/components/modal/BonusWallet/SlotsBonus/SwapSend.tsx";
import { SwapReceive } from "@/components/modal/BonusWallet/SportsBonus/SwapReceive.tsx";
import { userPurchaseSportsBonus } from "@/services/auth/sportsBonus";
...
import {
  useAvailableBalance, useSportSwapSelectedFirstTime
...
const SportsBonusSwapModal = ({
  ...
  const backend_currency = (currencies?.data ?? [])?.find((c: { currency: string; }) => c?.currency === currency_fiat);

  const min_deposit_require_value = formatCurrency({
    amount: convertCurrency({
      amount: parsed_data?.min_deposit_require_value || 0,
      fromCurrency: "USDT",
      toCurrency: bonusSwapFrom.currency?.currency ?? currency_fiat,
      exchangeRates
    }),
    currency: bonusSwapFrom.currency?.currency ?? currency_fiat
  });

  // TODO: 显示的最低购买要多一点
  const min_deposit_require_value_ceil = Decimal(min_deposit_require_value.value)
    .toDP(bonusSwapFrom.currency?.display_decimal ?? backend_currency?.display_decimal, Decimal.ROUND_CEIL)
    .toString();
  ...
  const createOrder = useCallback(async () => {
    ...
    try {
      const response = await userPurchaseSportsBonus(
