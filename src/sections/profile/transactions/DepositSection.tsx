...
import { TRANSACTION_PAGE_SIZE, useDepositRecords } from "@/query/transactions";
...
import { DepositList } from "./TransactionList";
...
export function DepositSection() {
  ...
  }), [baseParams, lastIdForCurrentPage]);

  const { data, isLoading, isFetching } = useDepositRecords(queryParams, { enabled: true });
    ...
    <>
      ...
      <DepositList
        ...
        onTransactionClick={(tx) => {
          setSelectedDetail(mapDetail({ transaction: tx, transactionType: "Deposit" as TransactionType, t }));
          setIsDetailsOpen(true);
        }}
      />

      <Paginate
        page={currentPage} limit={TRANSACTION_PAGE_SIZE} disabled={isFetching}
        pageCount={safeTotalPages}
        onJumpPage={setCurrentPage} onPaginate={setCurrentPage}
      />

      <TransactionDetailsDialog
        isOpen={isDetailsOpen && !!selectedDetail}
        onClose={() => { setIsDetailsOpen(false); setSelectedDetail(null); }}
