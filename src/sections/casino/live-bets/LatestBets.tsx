...
import { useAggregationConfig, useLatestBets } from "@/hooks/api/usePublic";
...
export const LatestBets = () => {
  ...
  const { data: latestBetsResponse } = useLatestBets();
  const { data: aggregationResponse } = useAggregationConfig();

  const { parsedMessages } = useMqttTopicMessagesReadonly(`public/order/latest_bet`);
  ...
  const mergedMessages = useMemo(() => {
    ...
    const apiList = (latestBetsResponse?.data ?? []).filter((item: any) => {
  ...
  }, [displayProviderWhitelist, latestBetsResponse, parsedMessages]);
